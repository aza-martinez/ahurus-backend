...
class PDFGenerator {
  ...
  async getPDF() {
    const templateHTML = fs.readFileSync(
      path.join(process.cwd(), CONFIG_PDFGENERATOR.pathTemplateExito),
      "utf-8"
    );
  ...
  async getPDFTrans() {
    ...
    const templateHTML = fs.readFileSync(
      path.join(process.cwd(), plantilla),
      "utf-8"
    );
  ...
  async getPDFDispersion() {
    ...
    const Total = this.transferencia.idTransferencia.reduce(
      (accumulator, current) =>
        parseFloat(parseFloat(accumulator) + parseFloat(current.monto)).toFixed(
          2
        ),
      0
    );
