...
var controller = {
        ...
        await request(
          ...
          async (error, response, body) => {
            ...
            const data = XLSX.utils.sheet_to_json(
              workbook.Sheets[sheet_name_list[sheetIndex - 1]]
            );
            ...
            dispersion.empresa =
              "SEFINCE"; /* req.user['http://localhost:3000/user_metadata'].empresa; */
            // INICIO FOREACH
            for await (let dato of data) {
              ...
              transferencia.tipoCuentaBeneficiario =
                dato.tipoCuentaBeneficiario;
              transferencia.tipoCuentaBeneficiario2 =
                dato.tipoCuentaBeneficiario2;
  ...
  ejecutar: async (req, res) => {
    ...
    try {
      ...
      if (!transferencias)
        return res.status(404).send("No se han encontrado resultados");
      ...
      const dispersion = await Dispersion.findOneAndUpdate(
        { _id: idDispersion },
